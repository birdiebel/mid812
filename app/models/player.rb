class Player < ApplicationRecord
  ...
  def self.ransackable_attributes(auth_object = nil)
    %w[created_at dob firstname id lang lastname sexe updated_at user_id]
  end
  ...
  def my_user
    self.user ? self.user.email : "<span class='is_red'>X</span>".html_safe
  end
  ...
  def age_category_id
    ...
    agecats.each do |agecat|
      return agecat.id if age >= agecat.age_low && age <= agecat.age_high
    end
  end
  ...
  def age_category
    ...
    agecats = Agecat.where(year: current_year).order(id: :desc)
    agecats.each do |agecat|
      return agecat.name if age >= agecat.age_low && age <= agecat.age_high
    end
    "N/A".html_safe
  end
  ...
  def icon_age_category
    ...
    agecats.each do |agecat|
      if age >= agecat.age_low && age <= agecat.age_high
        ...
        return (
          "<div class='bloc-agecat' style='background-color: #{age_color}; color: white'>
                        #{agecat.short}
                        </div>".html_safe
        )
      end
